...
class Blog{
    ...
    static async addNewBlog(user_id,title,description,creationDate){
        ...
        try {
            ...
            const addBlog= await db.collection(collectionName).insertOne({user_id:object_Id,title:title,description:description,creationDate:creationDate,lastUpdatedDate:creationDate})//in Mongo Cloud name is specified as User for DB
            return addBlog;
        } catch (error) {
            console.log(error);
    ...
    static async updateBlog(blog_id,user_id,title,description,lastUpdatedDate){
        const db=getDb();
        var object_Id=new mongoDb.ObjectID(blog_id);
        ...
        try {
            ...
            await deleteFromCache(key);
            const addBlog= await db.collection('Blog').updateOne({_id:object_Id},{$set : {title:title, description:description,lastUpdatedDate:lastUpdatedDate} })//in Mongo Cloud name is specified as User for DB
            return addBlog;
        } catch (error) {
            console.log(error);
            throw 'error DB query'
